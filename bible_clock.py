...
# Simple text wrapping function for the verse text.
def wrap_text(text, font, max_width):
    words = text.split()
    lines = []
    current_line = ""
    # We use a temporary image for text measurement
    temp_img = Image.new("P", (inky_display.WIDTH, inky_display.HEIGHT))
    temp_draw = ImageDraw.Draw(temp_img)

    for word in words:
        test_line = f"{current_line} {word}".strip()
        # Check if the line's width exceeds max_width
...
while True:
    # Create a new blank image canvas for each update.
    img = Image.new("P", (inky_display.WIDTH, inky_display.HEIGHT))
    draw = ImageDraw.Draw(img)

    # Reload Bible verses from JSON (to capture any updates)
    ...
    # Draw Bible verse reference (centered, bold)
    ...
    # Draw Bible verse text with word wrapping
    ...
    # Draw three cross symbols at the bottom
    cross_text = "✝   ✝   ✝"
    ...
    # Update the display with the composed image.
